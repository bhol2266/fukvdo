...
function Videos({ data, type }) {
    ...
    return (
        <div className="h-fit  basicMargin">
            <div className={`grid py-1 gap-1  ${viewType === 'horizontal' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-3 3xl:grid-cols-4 4xl:grid-cols-5' : 'grid-cols-2 lg:grid-cols-3 2xl:grid-cols-3 3xl:grid-cols-4 4xl:grid-cols-5'}`}>
                {
                    data.map(video => (
