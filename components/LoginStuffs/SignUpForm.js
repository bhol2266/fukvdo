...
export const SignUpForm = () => {
    ...
    const SignInButtonn = async (auth_provider) => {
        ...
        if (currentHost.includes("fuckvideo.live")) {
            const REDIRECT_URI2 = "https://www.fuckvideo.live/api/auth/fuckvideo/callback"
                    ...
                    <form className="space-y-3" action="#" method="POST" onSubmit={handleSubmit}>
                        ...
                        <div>
                            <button type="submit" className="relative mt-[20px] flex w-full h-[35px] justify-center rounded-md bg-gray-200 px-3 py-1.5 shadow-sm ">
                                ...
                                {!loading && <p className=" text-sm font-inter leading-6 text-semiblack font-inter font-semibold"> Join FuckVideo!</p>}
                            </button>
                        </div>
                    </form>
